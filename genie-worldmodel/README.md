...
> **Sandlot Sluggers Integration**: This copy has been adapted to work with Sandlot Sluggers baseball gameplay. See [Sandlot Sluggers Setup](#sandlot-sluggers-setup) below for game-specific instructions.
...
# Sandlot Sluggers Setup

## Recording Gameplay

**Option A — Browser download:**
```typescript
import { FrameCapture } from '../src/capture/FrameCapture';
const capture = new FrameCapture(engine.getRenderingCanvas()!, { fps: 30 });
capture.start();
// ... play the game ...
capture.stop();
await capture.exportMP4(); // downloads sandlot_sluggers.mp4
```

**Option B — Stream to capture server:**
```bash
python scripts/capture_server.py --output data/sandlot_sluggers.mp4
```
Then in the browser: `capture.streamToServer('ws://localhost:8765');`

## Training on Sandlot Sluggers Footage

```bash
# Place sandlot_sluggers.mp4 in genie-worldmodel/data/
python scripts/full_train.py --config configs/sandlot_sluggers_training.yaml
```

## Interactive Inference Server

```bash
python scripts/inference_server.py --config configs/sandlot_sluggers_inference.yaml
```

Connect from the game using `GenieClient`:
```typescript
import { GenieClient, GenieAction } from '../src/capture/GenieClient';
const client = new GenieClient('ws://localhost:8766');
await client.connect();
client.onFrame = (bitmap) => { ctx.drawImage(bitmap, 0, 0); };
client.sendAction(GenieAction.SWING);
```

## Action Space

The LAM discovers 8 discrete actions from gameplay (mapped approximately):

| ID | Action | ID | Action |
|----|--------|----|--------|
| 0 | Idle | 4 | Pitch Curveball |
| 1 | Swing | 5 | Pitch Slider |
| 2 | Bunt | 6 | Run |
| 3 | Pitch Fastball | 7 | Field |
