...
class SandlotSluggersDataset(VideoHDF5Dataset):
    """Dataset for Sandlot Sluggers baseball gameplay footage.

    Supports both recorded gameplay video (.mp4) and frame directories
    exported by the in-game frame capture system. The default resolution
    of 128x128 and 15 fps balance quality with tractable training times.

    To record gameplay:
      1. Enable frame capture in the browser game (see src/capture/FrameCapture.ts)
      2. Export frames as MP4 or HDF5
      3. Point video_path at the exported file
    """
    def __init__(self, video_path, transform=None, save_path=None, train=True, num_frames=4, resolution=(128, 128), fps=15, preload_ratio=1):
        super().__init__(
            video_path=video_path,
            ...
            load_chunk_size=1000,
            load_start_index=0,
            preprocess_read_step=2,  # keep every 2nd frame (game runs at 60fps, we want ~30)
            preprocess_slice=None,
        )
