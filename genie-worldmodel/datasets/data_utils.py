...
from datasets.datasets import PongDataset, SonicDataset, PolePositionDataset, PicoDoomDataset, ZeldaDataset, SandlotSluggersDataset
...
def load_sandlot_sluggers(num_frames=4, fps=15, preload_ratio=1):
    return _load_video_dataset_pair(
        SandlotSluggersDataset,
        '/data/sandlot_sluggers.mp4',
        '/data/sandlot_sluggers_frames.h5',
        num_frames=num_frames,
        fps=fps,
        preload_ratio=preload_ratio
    )
...
def load_data_and_data_loaders(dataset, batch_size, num_frames=1, distributed=False, rank=0, world_size=1, fps=15, preload_ratio=1):
    ...
    elif dataset == 'SANDLOT_SLUGGERS':
        training_data, validation_data = load_sandlot_sluggers(num_frames=num_frames, fps=fps, preload_ratio=preload_ratio)
    else:
